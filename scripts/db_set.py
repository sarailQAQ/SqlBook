...
def execsql(sql):
    url = 'http://127.0.0.1:8080/exec'
    d = {'sql': sql}
    r = requests.post(url, data=d)
    print(r.text)


def execsql_explain(sql):
    if not sql.strip().startswith('explain analyze'):
...
if __name__ == '__main__':
    ...
    execsql("""
    Insert into CSC(Sno, CNo, Grade) VALUES
    (201215121,1,92),
    (201215121,2,85),
    (201215121,3,88),
    (201215122,2,90),
    ...
    execsql("""
    Insert into CCourse(Cno, Cname,Cpno, Ccredit) VALUES
    (1,'Database',5,4),
    (2,'Math',NULL,2),
